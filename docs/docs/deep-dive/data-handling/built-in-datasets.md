...
!!! warning "This page is updated for DSPy 3.0 but may contain some outdated examples"

# Utilizing Built-in Datasets

## Installation

Starting with DSPy 3.0, dataset functionality requires additional dependencies that are not included in the core installation. To use DSPy's built-in datasets, you need to install the optional datasets dependencies:
...
If you encounter import errors when trying to use built-in datasets, make sure you have installed the optional dependencies:

```python
# This will fail without the datasets extra
from dspy.datasets import HotPotQA  # ImportError if datasets not installed
```

To resolve this, install the required dependencies as shown in the Installation section above.
