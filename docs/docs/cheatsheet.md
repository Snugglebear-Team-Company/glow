...
!!! warning "Updated for DSPy 3.0 - some examples may need verification"

# DSPy 3.0 Cheatsheet

This page contains snippets for frequent usage patterns in DSPy 3.0.

## Installation Requirements

DSPy 3.0 moved some functionality to optional dependencies. Install the extras you need:
...
```python
import dspy
from dspy.datasets import DataLoader  # Requires dspy[datasets]
...
## New DSPy 3.0 Features

### dspy.Audio

Handle base64-encoded audio data with multiple creation methods:

```python
# Create from URL
audio_from_url = dspy.Audio.from_url("https://example.com/audio.mp3")

# Create from file
audio_from_file = dspy.Audio.from_file("audio.wav")

# Create from array
import numpy as np
audio_array = np.random.rand(1000)
audio_from_array = dspy.Audio.from_array(audio_array)

# Display metadata
print(audio_from_file.metadata)
```

### XMLAdapter

Handle XML-formatted inputs and outputs:
...
parsed_data = xml_adapter.parse(xml_string)

# Format data as XML
data = {"name": "Jane", "age": 25}
xml_output = xml_adapter.format(data)
```

### CodeAct Module

Generate and execute Python code with DSPy:
...
result = code_act(question=question)
...
from langchain.tools import some_langchain_tool

# Convert LangChain tool to DSPy Tool
dspy_tool = Tool.from_langchain(some_langchain_tool)

# Use with ToolCalls and OpenAI-compatible JSON schemas
tool_calls = dspy.ToolCalls([dspy_tool])
schema = tool_calls.format()  # OpenAI-compatible JSON schema
```

### Streaming Support

Enable streaming in both async and sync contexts:

```python
# Configure async streaming
dspy.settings.configure(async_streaming=True)

# Use sync wrapper utilities
from dspy.utils import apply_sync_streaming
...
### Custom Types with BaseType

Define custom content types for embedding in messages:

```python
from dspy import BaseType

class CustomImageType(BaseType):
    def __init__(self, image_data):
        self.image_data = image_data
    
    def split_content(self):
        # Custom splitting logic for images
        return [self.image_data]

# Use custom type in messages
custom_image = CustomImageType(image_data="base64_encoded_image")
```
...
## DSPy 3.0 Configuration Changes

### Updated Default Settings

DSPy 3.0 includes several updated defaults for better performance:

```python
# Default max_tokens increased from 1000 to 4000
dspy.settings.configure(max_tokens=4000)  # New default

# Default LM retry attempts reduced from 8 to 3
dspy.settings.configure(max_retry_attempts=3)  # New default

# Enable async streaming (new feature)
dspy.settings.configure(async_streaming=True)
```

### Enhanced Error Handling

DSPy 3.0 provides improved error messages and warnings:

```python
# Warnings for truncated outputs
# DSPy will now warn when LM outputs are truncated due to token limits

# Better error messages for incorrect usage
try:
    predictor = dspy.Predict(BasicQA, "invalid_positional_arg")
except RuntimeError as e:
    print(f"Improved error message: {e}")

# Warnings for direct forward() calls
# DSPy will warn when calling forward() methods directly
# Use instance invocation instead: predictor(question="...") 
```